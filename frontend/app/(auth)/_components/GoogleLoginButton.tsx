...
export function GoogleLoginButton() {
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      toast.error("Could not reach server. Check that the backend is running.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={loading}
      className="h-11 w-full rounded-lg border bg-background text-[#3C4043] font-semibold hover:bg-muted/50 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
    >
      {loading ? (
