...
exports.AiScanService = void 0;
const tesseract_js_1 = require("tesseract.js");
const normalizeRecordType = (recordType) => recordType?.trim().toLowerCase();
...
const parseVitals = (text) => {
    ...
    const hrMatch = text.match(/(?:heart rate|hr)\s*[:\-]?\s*(\d{2,3})/i) ||
        text.match(/(\d{2,3})\s*bpm/i);
    if (hrMatch) {
        result.heartRate = parseNumber(hrMatch[1]);
    }
    const glucoseMatch = text.match(/(?:glucose|blood sugar)\s*[:\-]?\s*(\d{2,3}(?:\.\d+)?)/i);
    if (glucoseMatch) {
...
const buildSummary = (recordType, structured, text) => {
    ...
    const lines = (text ?? "")
        ...
        .filter((line) => !/(patient details|s\.no|testname|interpretation|normal range|consult doctor)/i.test(line));
    if (lines.length) {
...
const extractProvider = (text) => {
    const urlMatch = text.match(/\b(?:https?:\/\/)?(?:www\.)?([a-z0-9.-]+\.[a-z]{2,})\b/i);
    if (urlMatch?.[1]) {
    ...
    const lines = text
        ...
        .map((line) => line.trim())
        .filter(Boolean);
    const providerLine = lines.find((line) => /(hospital|clinic|lab|diagnostic|diagnostics|medical|health center|health centre)/i.test(line));
    return providerLine;
};
const isLikelyBadProvider = (value) => {
    if (!value)
        return true;
    ...
    if (badTokens.some((token) => normalized.includes(token)))
        return true;
...
const buildExtractionPrompt = (text, recordType) => {
...
class AiScanService {
    async scanImage(buffer, recordType) {
        ...
        if (!Object.keys(structured).length) {
            ...
            if (fallbackType === "vitals")
                structured = parseVitals(normalizedText);
            if (fallbackType === "lab")
                structured = parseLab(normalizedText);
        }
        if (!provider) {
        ...
        if (!recordDate)
            recordDate = extractDate(normalizedText);
...
exports.AiScanService = AiScanService;
